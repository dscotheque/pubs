...
NOTE: This is a patched version of labpubs/dedup.py.  The upstream
merge_works() uses an "existing wins" policy that preserves sparse
metadata from early sources (e.g. Google Scholar alerts) even when a
richer source (e.g. OpenAlex) arrives later.  This patch changes
title, authors, and venue to prefer the **richer** value.
"""
...
def _pick_richer_str(a: str | None, b: str | None) -> str | None:
    ...
    if not b:
        return a
    # Penalise truncated strings (trailing ellipsis)
...
def merge_works(existing: Work, new: Work) -> Work:
    ...
    Fills missing fields from the new source and, for title, authors,
    and venue, prefers the **richer** value (longer / non-truncated /
    more complete).  Always updates citation counts and adds new
    source IDs.
